...
module App
  class Application < Rails::Application
    ...
    config.autoload_paths << Rails.root.join('app', 'graphql', 'instruments')
    ...
    origins = ENV['ACTION_CABLE_ALLOWED_REQUEST_ORIGINS'].split(',') if ENV['ACTION_CABLE_ALLOWED_REQUEST_ORIGINS'].present?
    origins ||= []
    origins.map! { |url| /#{url}/ }
    config.action_cable.allowed_request_origins = origins

    # URL Options
    if ENV.key? "DEFAULT_URL_HOST"
