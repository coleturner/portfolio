module Resolvers
  class Base < ::GraphQL::Rails::Resolver
    ...
    def call(obj, args, ctx)
      ...
      if has_id_argument? and args.key? @id_field.to_s
        @result = resolve_id(args[@id_field.to_s])
      end

      @resolvers.each do |arg, resolvers|
        if args.key? arg
          original_value = args[arg.to_s]
          ...
          resolvers.each do |definition, params|
            ...
            # Match custom methods
              ...
              @result = resolve_argument_value(@result, argument: arg, value: value, params: params)
            end
          end
        end
      end
    ...
    def resolve_argument_value(result, argument:, value:, params:)
      ...
      if is_arg_id_type?(argument) && !value.nil?
    ...
    def get_field_args
      @ctx.irep_node.definitions.first.arguments
    end

    def get_arg_type(key)
      args = get_field_args

      if args[key.to_s].nil?
        raise ArgumentError, "Argument #{key} does not exist on #{field_name}"
      end
