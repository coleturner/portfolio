...
import { PORTRAIT_ID, CONTENTFUL_HOST } from './constants';
...
function toContentfulPortraitURL(urlStr) {
...
export async function getPortraitURL() {
  const portrait = await client.getAsset(PORTRAIT_ID);

  const portraitURL =
    toContentfulPortraitURL(portrait?.fields?.file?.url) || '/portrait.jpg';

  return portraitURL;
}

export function getFormattedImageURL(urlStr, params) {
