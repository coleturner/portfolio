...
function Anchor({ children }) {
  ...
  const anchorText = toSlug(anchorChildren.join('-'));

  if (!anchorText) {
    return children;
  }

  const childrenWithLinks = React.Children.map(children, (n) => {
    if (typeof n === 'string') {
      return <a href={'#' + anchorText}>{n}</a>;
    }

    if (
      typeof n.props.children === 'string' &&
      !n.props.href & (n.type !== 'a')
    ) {
      return React.cloneElement(n, {
        ...n.props,
        children: <a href={'#' + anchorText}>{n.props.children}</a>,
      });
    }
  ...
  return (
    <AnchorLinkContainer>
      <span className="anchor" id={anchorText}></span>
      {childrenWithLinks}
